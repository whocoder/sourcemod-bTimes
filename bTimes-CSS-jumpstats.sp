...
ConVar Cvar_Show_Message;
ConVar Cvar_Invalid_Jump;
ConVar Cvar_Stop_LJ;
ConVar Cvar_Stop_BJ;
ConVar Cvar_Stop_BUJ;
ConVar Cvar_Stop_Block_LJ;


ConVar Cvar_Print_LJ;

ConVar Cvar_Print_WJ;

ConVar Cvar_Print_BJ;

ConVar Cvar_Print_Block_LJ;

ConVar Cvar_Print_Block_BJ;

ConVar Cvar_DB_Record;

new Handle:Cvar_Sound[5];
new Float:g_Sound[5];
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max){
	RegPluginLibrary("bTimes-jumpstats");
	g_bLateLoaded = late;
...
public OnPluginStart(){
	...
	Cvar_Stop_LJ 			= CreateConVar("timer_ljstats_print_lj", "240.0", "Long Jump minimum units to show result to player.", FCVAR_PLUGIN);
	Cvar_Stop_BJ 			= CreateConVar("timer_ljstats_print_bj", "270.0", "Bhop Jump minimum units to show result to player.", FCVAR_PLUGIN);
	Cvar_Stop_BUJ 			= CreateConVar("timer_ljstats_print_buj", "250.0", "Bhop Up Jump minimum units to show result to player.", FCVAR_PLUGIN);
	Cvar_Stop_Block_LJ 		= CreateConVar("timer_ljstats_register_block_lj", "235.0", "Minimum units to register Block Long Jump.", FCVAR_PLUGIN);
	Cvar_Print_LJ			= CreateConVar("timer_ljstats_show_lj", "260.0", "Print long jump result over this value.", FCVAR_PLUGIN);
	Cvar_Print_WJ 			= CreateConVar("timer_ljstats_show_wj", "270.0", "Print weird jump result over this value.", FCVAR_PLUGIN);
	Cvar_Print_BJ 			= CreateConVar("timer_ljstats_show_bj", "270.0", "Print bhop Jump result over this value.", FCVAR_PLUGIN);
	Cvar_Print_Block_LJ 		= CreateConVar("timer_ljstats_show_block_lj", "255.0", "Print Block Long Jump result over this value.", FCVAR_PLUGIN);
	Cvar_Print_Block_BJ 		= CreateConVar("timer_ljstats_show_block_bj", "275.0", "Print Block Bhop Jump result over this value.", FCVAR_PLUGIN);
	Cvar_DB_Record 			= CreateConVar("timer_ljstats_db_record", "26000.0", "Record result into database over this value.", FCVAR_PLUGIN);
	Cvar_Sound[0] 			= CreateConVar("timer_ljstats_snd_impressive", "255.0", "Play Sound - impressive.", FCVAR_PLUGIN);
	Cvar_Sound[1] 			= CreateConVar("timer_ljstats_snd_perfect", "260.0", "Play Sound - perfect.", FCVAR_PLUGIN);
...
public OnAutoConfigsBuffered(){
	for(new i = 0; i < sizeof(g_Sound); i++)
	{
		g_Sound[i] = GetConVarFloat(Cvar_Sound[i]);
	}
	ConnectToDB();
}
...
public OnClientDisconnect(client){
	//(GetClientSettings(client) & JUMP_STATS) = false;
...
public Action:Event_PlayerJump(Handle:event, const String:name[], bool:Broadcast){
...
CalculateBlockGap(client, Float:origin[3], Float:target[3])
{
	...
	{
		PrintToChat(client, "\x01[\x05LJstats\x01]\x03You can regiter block more than %.0f units.", Cvar_Stop_Block_LJ.FloatValue);
	}
}
stock CalculateBlockGap2(client, Float:origin[2][3], Float:target[2][3])
{
	...
	{
		PrintToChat(client, "\x01[\x05LJstats\x01]\x03You can regiter block more than %.0f units. (\x01�\x03)", Cvar_Stop_Block_LJ.FloatValue);
	}
...
public Action:Command_ToggleLJ(client, args){
	...
	if(GetClientSettings(client) & JUMP_STATS)
		...
		PrintColorText(client, "\x01[\x05LJstats\x01]\x03 Longjump stats \x01disabled\x03.");
}
...
public Action:Command_MeasureGap(client, args)
{
	if(0 < client <= MaxClients)
	{
		for(new i = 0; i < 3; i++)
...
StartJumpStats(client, JumpType:type, Float:pos[3], Float:vel)
{
	...
	if(!StrEqual(weapon_name, "weapon_ak47") && !StrEqual(weapon_name, "weapon_m4a1") && !StrEqual(weapon_name, "weapon_awp"))
	{
		PrintToChat(client, "\x01[\x05LJstats\x01]\x03Wrong : \x01 Check your weapon (Use a AK47, M4A1, or AWP).");
		return;
	}
	...
	if(gravity != 0.0&&gravity!=1.0)
	{
		if(GetClientSettings(client) & JUMP_STATS){
			SetClientSettings(client, GetClientSettings(client) & ~JUMP_STATS);
			PrintToChat(client, "\x01[\x05LJstats\x01]\x03LJmode disabled : \x01 Check Gravity.");
		}
	}
	ResetLJStats(client);
...
public Action:LJStats(client, bool:fail)
{
	...
	{/**Compare jumped and landed heights. although tried to get correct z-coord by custom function. still has an error.
		...
		else
		{
			if(J_type == JumpType_LongJump)
			{
				...
				if(PreStrafe[client] >= Cvar_Invalid_Jump.FloatValue||distance >= Cvar_Invalid_Jump.FloatValue)
				{
					PrintToChat(client, "\x01[\x05LJstats\x01]\x03Wrong jump.");
					return Plugin_Handled;
				}
				if(distance <= Cvar_Stop_LJ.FloatValue && !fail)
				{
					return Plugin_Handled;
				}
			}
			...
			{
				if(distance < Cvar_Stop_BJ.FloatValue)
				{
					return Plugin_Handled;
				}
			}
			...
			{
				if(distance < Cvar_Stop_BUJ.FloatValue)
				{
					return Plugin_Handled;
				}
			}
	...
	{
		if(distance >= Cvar_Print_WJ.FloatValue)
		{
	...
	{
		if(distance >= Cvar_Print_LJ.FloatValue)
		{
			PrintToChatLJ("\x04[\x01%s\x04]\x03%N - \x01%.3f\x04units!",JumpName[J_type],client,distance);
	...
	{
		if(blockdistance >= Cvar_Print_Block_LJ.FloatValue)
		{
	...
	{
		if(blockdistance >= Cvar_Print_Block_BJ.FloatValue)
		{
			PrintToChatLJ("\x04[\x01%s\x04]\x03%N - \x01%.0f\x04units!\n (actual : %.3f, Edge : %.3f)",JumpName[J_type],client,blockdistance, distance, g_EdgeDist[client]);
		}
	}
	...
	{
		if(distance >= Cvar_Print_BJ.FloatValue)
		{
...
stock PrintToChatLJ(const String:msg[], any:...)
{
	...
	for(new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))
		{
			if(Cvar_Show_Message.IntValue == 0)
			{
			...
			{
				if(GetClientSettings(i) & JUMP_STATS)
				{
					SetGlobalTransTarget(i);
					VFormat(buffer, sizeof(buffer), msg, 2);
					PrintToChat(i, buffer);
...
ConnectToDB(){
	...
	Cvar_DB.GetString(dbname, sizeof(dbname));
	if(strlen(dbname) > 0)
	{
		SQL_TConnect(getmaindb, dbname);
	}
}
public getmaindb(Handle:owner, Handle:dbhandle, const String:error[], any:data){//check current connection. and data tables
...
public Action:Command_LJAdmin(client, args)
{
	if(client != 0)
	{
		AdmTopMenu(client);
