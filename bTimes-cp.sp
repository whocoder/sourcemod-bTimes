...
public OnPluginStart(){
	...
	RegAdminCmd("sm_goto", SM_GoTo, ADMFLAG_CUSTOM5, "Allows admins to force sm_tpto to a player.");
...
public Action:SM_GoTo(client, args)
{
	if(IsPlayerAlive(client))
	{
		if(args == 0)
		{
			PrintColorText(client, "%s%sYou must specify a user.",
				g_msg_start,
				g_msg_textcol);
		...
		else
		{
			...
			if(client != target)
			{
				if(target != -1)
				{
					...
					PrintColorText(client, "%s%sTarget not found.",
							g_msg_start,
							g_msg_textcol);
				}
			}else{
...
TeleportToCheckpoint(client, cpnum)
{
	if(GetConVarBool(g_hAllowCp))
	{
		if(0 <= cpnum < g_cpcount[client])
		{
			...
			{
				new String:styleAbbreviation[32];
				GetStyleAbbr(GetClientStyle(client), styleAbbreviation, sizeof(styleAbbreviation), true);
				new Float:vClientPos[3];
				Entity_GetAbsOrigin(client, vClientPos);
				
				if(StrEqual(styleAbbreviation, "ps")){
				...
				else if(Timer_InsideZone(client, MAIN_START) != -1 || Timer_InsideZone(client, BONUS_START) != -1)
				{
					TeleportEntity(client, 
						g_UsePos[client]?g_cp[client][cpnum][0]:NULL_VECTOR, 
						g_UseAng[client]?g_cp[client][cpnum][2]:NULL_VECTOR, 
			...
			else
			{
				...
				else if(!Timer_IsPointInsideZone(vPos, MAIN_START, 0) && !Timer_IsPointInsideZone(vPos, BONUS_START, 0))
				{
					TeleportEntity(client, 
						g_UsePos[client]?g_cp[client][cpnum][0]:NULL_VECTOR, 
						g_UseAng[client]?g_cp[client][cpnum][2]:NULL_VECTOR, 
