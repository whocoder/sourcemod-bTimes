		...
		for(new j=1; j<=7; j+=(j/2)+1)
		{
			if(j != 7-i)
			{
				...
				if(GetGameType() == GameType_CSS)
					...
					TE_SetupBeamPoints(g_Zones[Zone][ZoneNumber][i], g_Zones[Zone][ZoneNumber][j], g_Properties[Zone][ModelIndex], g_Properties[Zone][HaloIndex], 0, 0, life, 1.0, 1.0, 10, 0.0, color, 0);
				#endif
