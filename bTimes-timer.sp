...
StyleConfig g_StyleConfig[MAX_STYLES][StyleConfig];
new g_TotalStyles;
...
// Cvars
Handle g_hTimerDisplay = INVALID_HANDLE;

ConVar Cvar_Hintspeed;
ConVar Cvar_AllowYawspeed;
ConVar Cvar_AllowPause;
ConVar Cvar_ChangeClanTag;
ConVar Cvar_ShowTimeLeft;
ConVar Cvar_AdvancedSounds;
ConVar Cvar_AllowNoClip;
ConVar Cvar_VelocityCap;
ConVar Cvar_AllowAuto;
ConVar Cvar_JumpInStartZone;
ConVar Cvar_AutoStopsTimer;
...
float g_fPlayerSync[MAXPLAYERS+1];
...
public OnPluginStart(){
	...
	// Server cvars
	Cvar_Hintspeed       = CreateConVar("timer_hintspeed", "0.1", "Changes the hint text update speed (bottom center text)", 0, true, 0.1);
	Cvar_AllowYawspeed   = CreateConVar("timer_allowyawspeed", "0", "Lets players use +left/+right commands without stopping their timer.", 0, true, 0.0, true, 1.0);
	Cvar_AllowPause      = CreateConVar("timer_allowpausing", "1", "Lets players use the !pause/!unpause commands.", 0, true, 0.0, true, 1.0);
	Cvar_ChangeClanTag   = CreateConVar("timer_changeclantag", "1", "Means player clan tags will show their current timer time.", 0, true, 0.0, true, 1.0);
	Cvar_ShowTimeLeft    = CreateConVar("timer_showtimeleft", "1", "Shows the time left until a map change on the right side of player screens.", 0, true, 0.0, true, 1.0);
	Cvar_AdvancedSounds  = CreateConVar("timer_advancedsounds", "1", "Reads record sound options from sounds.txt", 0, true, 0.0, true, 1.0);
	Cvar_AllowNoClip     = CreateConVar("timer_noclip", "1", "Allows players to use the !p commands to noclip themselves.", 0, true, 0.0, true, 1.0);
	Cvar_VelocityCap     = CreateConVar("timer_velocitycap", "1", "Allows styles with a max velocity cap to cap player velocity.", 0, true, 0.0, true, 1.0);
	Cvar_JumpInStartZone = CreateConVar("timer_allowjumpinstart", "1", "Allows players to jump in the start zone. (This is not exactly anti-prespeed)", 0, true, 0.0, true, 1.0);
	Cvar_AllowAuto       = CreateConVar("timer_allowauto", "1", "Allows players to use auto bunnyhop.", 0, true, 0.0, true, 1.0);
	Cvar_AutoStopsTimer  = CreateConVar("timer_autostopstimer", "0", "Players can't get times with autohop on.");

	HookConVarChange(Cvar_Hintspeed, OnTimerHintSpeedChanged);
	HookConVarChange(Cvar_AutoStopsTimer, OnAutoStopsTimerChanged);
	...
	HookEvent("player_changename", Event_UpdateNames, EventHookMode_Post);

	// Admin commands
	RegAdminCmd("sm_delete", SM_Delete, ADMFLAG_CUSTOM5, "Deletes map times.");
	RegAdminCmd("sm_spj", SM_SPJ, ADMFLAG_CUSTOM5, "Check the strafes per jump ratios for any player.");
	...
	RegConsoleCmdEx("sm_modes", SM_Style, "Change your style.");
	...
	RegConsoleCmdEx("sm_fullhud", SM_Fullhud, "Toggles normal or simple HUD display.");
	RegConsoleCmdEx("sm_simplehud", SM_Fullhud, "Toggles normal or simple HUD display.");
	RegConsoleCmdEx("sm_maxinfo", SM_Fullhud, "Toggles normal or simple HUD display.");
	RegConsoleCmdEx("sm_hidehud", SM_HideHud, "Toggles the center-HUD display.");
	RegConsoleCmdEx("sm_disablehud", SM_HideHud, "Toggles the center-HUD display.");
	RegConsoleCmdEx("sm_hud", SM_Hud, "Change what shows up on the right side of your hud.");
	RegConsoleCmdEx("sm_2dvel", SM_TrueVelocity, "Toggles between 2D and 3D velocity velocity meters");
	...
	RegConsoleCmd("+strafe", SM_BlockStrafe);
	RegConsoleCmd("-strafe", SM_BlockStrafe);
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("Style_CalculateStrafeStats", Native_Style_CalculateStrafeStats);
...
public OnMapStart()
{
	...
	CreateTimer(1.0, SetClanTag, _, TIMER_FLAG_NO_MAPCHANGE | TIMER_REPEAT);

	if(GetGameType() == GameType_CSGO)
		CreateTimer(0.2, Timer_UpdateSync, _, TIMER_FLAG_NO_MAPCHANGE | TIMER_REPEAT);
}

public OnStylesLoaded()
{
	RegConsoleCmdPerStyle("record", SM_WorldRecord, "Show the world record info for {Type} timer on {Style} style.");
	...
	RegConsoleCmdPerStyle("pr", SM_Time, "Show your time for {Type} timer on {Style} style.");
	RegConsoleCmdPerStyle("me", SM_Time, "Show your time for {Type} timer on {Style} style.");
...
public OnConfigsExecuted()
{
	...
	if(Cvar_AdvancedSounds.BoolValue)
	{
	...
	g_hTimerDisplay = CreateTimer(Cvar_Hintspeed.FloatValue, Timer_DrawHintText, _, TIMER_FLAG_NO_MAPCHANGE | TIMER_REPEAT);
...
public Action:Event_PlayerJump_Post(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(Cvar_VelocityCap.BoolValue == true)
	{
		new client = GetClientOfUserId(GetEventInt(event, "userid"));

		new Style = g_Style[client][g_Type[client]];

		if(Cvar_VelocityCap.BoolValue == true && g_StyleConfig[Style][MaxVel] != 0.0)
		{
...
public Action:SM_Auto(client, args){
	if(Cvar_AllowAuto.BoolValue == true)
	{
		if (args < 1){
			SetClientSettings(client, GetClientSettings(client) ^ AUTO_BHOP);

			if(Cvar_AutoStopsTimer.BoolValue && (GetClientSettings(client) & AUTO_BHOP)){
				StopTimer(client);
			}
...
// Toggles if the display area is enabled
public Action:SM_HideHud(client, args){
	SetClientSettings(client, GetClientSettings(client) ^ DISABLE_HUD);

	if(GetClientSettings(client) & DISABLE_HUD)
	{
		PrintColorText(client, "%s%sYour center HUD will no longer be displayed.",
			g_msg_start,
			g_msg_textcol);
	}
	else
	{
		PrintColorText(client, "%s%sYour center HUD has been enabled.",
			g_msg_start,
			g_msg_textcol);
	}
...
OpenHudMenu(client)
{
	...
	SetMenuTitle(menu, "HUD Settings");
...
public Action:SetClanTag(Handle:timer, any:data){
	if(Cvar_ChangeClanTag.IntValue == 1){
		decl String:sTag[32];
		for(new client = 1; client <= MaxClients; client++)
		{
...
public Action Timer_UpdateSync(Handle timer, any data)
{
	for(int client = 1; client <= MaxClients; client++)
	{
		if(IsClientInGame(client) && !IsFakeClient(client) && IsPlayerAlive(client))
			g_fPlayerSync[client] = GetClientSync(client);
	}
}
...
GetClanTagString(client, String:tag[], maxlength){
	if(g_bTiming[client] == true){
		if(Timer_InsideZone(client, MAIN_START, -1) != -1 || Timer_InsideZone(client, BONUS_START, -1) != -1){
			...
			new Float:fPausedTime = g_fPauseTime[client] - g_fStartTime[client];

			decl String:sPausedTime[32];
			FormatPlayerTime(fPausedTime, sPausedTime, sizeof(sPausedTime), false, 0);
			FormatEx(tag, maxlength, "PAUSED %s", sPausedTime);
			return;
		}else{
			...
			FormatPlayerTime(fTime, sTime, sizeof(sTime), false, 0, true);
		...
		if(IsClientInGame(client) && !IsFakeClient(client))
		{
			...
			if(GetHintMessage(client, sHintMessage, sizeof(sHintMessage))){
				if((GetGameType() == GameType_CSGO && !(GetClientSettings(client) & SHOW_KEYS)))
					PrintHintGameText(client, sHintMessage);
...
bool:GetHintMessage(client, String:buffer[], maxlength)
{
	...
	if(Timer_InsideZone(target, MAIN_START) != -1 || Timer_InsideZone(target, BONUS_START) != -1){
		if(GetGameType() != GameType_CSGO){
			FormatEx(buffer, maxlength, "%sIn Start Zone\n \n\nSpeed: %d", buffer,
				RoundToFloor(GetClientVelocity(target, true, true, !bool:(settings & SHOW_2DVEL))));
		}
		else
		{
			// Green text for Main Start
			// Blue text for Bonus Start
			if(Timer_InsideZone(target, MAIN_START) != -1)
				FormatEx(buffer, maxlength, "	<font size='40' color='#00FF00'>In Start Zone</font>");
			else
				FormatEx(buffer, maxlength, "	<font size='40' color='#0000FF'>In Start Zone</font>");

			FormatEx(buffer, maxlength, "%s\nVelo: %d				Spectators: %d", buffer,
				RoundToFloor(GetClientVelocity(target, true, true, !bool:(settings & SHOW_2DVEL))),
	...
	else
	{
		if(g_bTiming[target])
		{
			...
			{
				if(GetGameType() == GameType_CSS || (settings & SHOW_HINT))
				{
		...
		else
		{
			if(GetGameType() == GameType_CSGO)
			{
				FormatEx(buffer, maxlength, "	   <font size='40' color='#ff0000'>NO TIMER</font>\nVelo: %d				Spectators: %d",
					RoundToFloor(GetClientVelocity(target, true, true, !bool:(settings & SHOW_2DVEL))),
					(g_bIsAdmin[client])?AdminSpecCount[target]:SpecCount[target]);
			}
			else if (GetGameType() == GameType_CSS)
			{
				FormatEx(buffer, maxlength, "%sNO TIMER\n\n\nSpeed: %d", buffer,
					RoundToFloor(GetClientVelocity(target, true, true, !bool:(settings & SHOW_2DVEL))));
			}
		}
	}
	...
	if(GetGameType() == GameType_CSS){
		...
		if(g_StyleConfig[Style][Hud_Style]){
			if(g_Type[client] == TIMER_BONUS)
				...
				Format(sResult, maxlength, "%s", g_StyleConfig[Style][Name]);

			if(g_StyleConfig[Style][Freestyle] && (Timer_InsideZone(client, FREESTYLE, 1 << Style) != -1))
		...
		Format(sResult, maxlength, "%sSpeed: %d", sResult, RoundToFloor(GetClientVelocity(client, true, true, (GetClientSettings(target) & SHOW_2DVEL) == 0)));
	}else if(GetGameType() == GameType_CSGO && !(GetClientSettings(client) & SHOW_KEYS)){
		// Timer HUD for CSGO (MINOR HTML :D)

		FormatPlayerTime(fTime, sTime, sizeof(sTime), false, 0, true);

		// Display player's time and their relative possible rank on map
		FormatEx(sResult, maxlength, "<plaintext>Time: <font color='#990033'>%s</font>", sTime);
		FormatEx(sResult, maxlength, "%s		(#%d)", sResult, GetPlayerPosition(fTime, g_Type[client], Style));

		if(g_StyleConfig[Style][Freestyle] && (Timer_InsideZone(client, FREESTYLE, 1 << Style) != -1))
			FormatEx(sResult, maxlength, "%s <font color='#00ffff'>(FREESTYLE)</font>\n", sResult);
		else
			FormatEx(sResult, maxlength, "%s\n", sResult);

		// If a player wants to display their sync
		if(GetClientSettings(client) & KH_SYNC)
		{
			float PlayerSync = g_fPlayerSync[client];

			if(g_StyleConfig[Style][CalcSync] && PlayerSync != 0.0)
			{
				if(PlayerSync > 99.99)
					FormatEx(sResult, maxlength, "%sSync: 99.00", sResult);
				else
					FormatEx(sResult, maxlength, "%sSync: %.2f", sResult, PlayerSync);
			}
			else
			{
				FormatEx(sResult, maxlength, "%sSync: --", sResult);
			}
			...
			FormatEx(sResult, maxlength, "%s			", sResult);
			FormatEx(sResult, maxlength, "%sVelo: %d\n", sResult, RoundToFloor(GetClientVelocity(client, true, true, (GetClientSettings(target) & SHOW_2DVEL) == 0)));

			if(GetClientSettings(client) & KH_SPECS){
				FormatEx(sResult, maxlength, "%sSpectators: %d", sResult, (g_bIsAdmin[client])?AdminSpecCount[target]:SpecCount[target]);
				FormatEx(sResult, maxlength, "%s		", sResult);
			}
		}
		else
		{
			FormatEx(sResult, maxlength, "%sStrafes: %d", sResult, g_Strafes[client]);
			FormatEx(sResult, maxlength, "%s			", sResult);
			FormatEx(sResult, maxlength, "%sJumps: %d\n", sResult, g_Jumps[client]);
			FormatEx(sResult, maxlength, "%sVelo: %d", sResult, RoundToFloor(GetClientVelocity(client, true, true, (GetClientSettings(target) & SHOW_2DVEL) == 0)));
			FormatEx(sResult, maxlength, "%s			", sResult);

			if(GetClientSettings(client) & KH_SPECS){
				FormatEx(sResult, maxlength, "%sSpectators: %d\n", sResult, (g_bIsAdmin[client])?AdminSpecCount[target]:SpecCount[target]);
			}
		...
		if(g_StyleConfig[Style][Hud_Style]){
			if(g_Type[client] == TIMER_BONUS)
				FormatEx(sResult, maxlength, "%sStyle: Bonus", sResult);
			else
				FormatEx(sResult, maxlength, "%sStyle: %s", sResult, g_StyleConfig[Style][Name]);
		}

		FormatEx(sResult, maxlength, "%s</plaintext>", sResult);
	}
...
GetTimerSimpleString(client, String:sResult[], maxlength)
{
	...
	if(GetGameType() == GameType_CSGO)
	{
		...
		Format(sResult, maxlength, "	<font size='50' color='#800080'>%s</font>", sTime);
	}
	else
	{
		FormatPlayerTime(fTime, sTime, sizeof(sTime), false, 0);
...
public OnRadarAlphaRetrieved(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	if(StringToInt(cvarValue) == 0)
	{
		...
		if(GetSyncHudMessage(client, sMessage, sizeof(sMessage)))
		{
			...
			if(hText != INVALID_HANDLE)
			{
				SetHudTextParams(0.01, 0.01, 1.0, 255, 0, 255, 255);
...
bool:GetSyncHudMessage(client, String:message[], maxlength)
{
	...
	if(!IsFakeClient(target))
	{
		...
		{
			new itSize = GetArraySize(g_hTimes[Type][Style]);
			position = GetPlayerPositionByID(GetPlayerID(target), g_Type[target], GetStyle(target));
			Format(message, maxlength, "%s (#%d / %d)", message, position, itSize);
		}
...
bool:GetKeyHintMessage(client, String:message[], maxlength, SpecCount[], AdminSpecCount[])
{
	...
	if(settings & KH_TIMELEFT)
	{
		...
		if(Cvar_ShowTimeLeft.BoolValue && timelimit != 0)
		{
			new timeleft;
...
public Action:OnTimerStart_Pre(client, Type, Style)
{
	...
	{
		SetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue", 1.0);
		//WarnClient(client, "%s%sYour movement speed is off. It has been.", 30.0,
		//	g_msg_start,
		//	g_msg_textcol,
		//	g_msg_varcol,
		//	g_msg_textcol);
	...
	if((GetClientSettings(client) & AUTO_BHOP) && Cvar_AutoStopsTimer.BoolValue)
	{
		return Plugin_Handled;
	}
	...
	if(Cvar_AdvancedSounds.BoolValue)
	{
		if(NewTime == true)
		{
			...
			if(iSize > 0)
			{
				...
				EmitSoundAny(clients, numClients, sSound);
			}
			else
			{
				...
				if(iSize > 0)
				{
					...
					if(!(GetClientSettings(client) & STOP_PBSND))
						EmitSoundToClientAny(client, sSound);
				}
			}
...
public int Native_Style_CalculateStrafeStats(Handle plugin, int numParams){
	int Style = GetNativeCell(1);
	return (g_StyleConfig[Style][Count_Left_Strafe] && g_StyleConfig[Style][Count_Right_Strafe] && g_StyleConfig[Style][CalcSync]);
}
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		if(rowcount != 0)
		{
			...
			for(new i = 1; i <= RowCount; i++)
			{
				...
				if(GetGameType() != GameType_CSGO && ((i % 7) == 0 || i == RowCount))
			...
			if(ClientTime != 0.0)
			{
				...
				FormatEx(title, sizeof(title), "%s records\n[%s] - [%s]\nYour time: %s ( %d / %d )\n--------------------------------------",
					sMapName,
					sType,
					sStyle,
					sClientTime,
...
DB_LoadTimes(bool:FirstTime)
{
	...
	if(FirstTime)
		SQL_TQuery(g_DB, LoadTimes_Callback, query, pack, DBPrio_High);
	else
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if(IsPlayerAlive(client))
	{
		...
		// Check gravity
		if(g_StyleConfig[Style][Gravity] != 0.0 && g_StyleConfig[Style][Gravity] != 1.0)
		{
			if(GetEntityGravity(client) > g_StyleConfig[Style][Gravity])
				SetEntityGravity(client, g_StyleConfig[Style][Gravity]);
		...
		// Anti - +left/+right
		if(Cvar_AllowYawspeed.BoolValue == false)
		{
			if(buttons & (IN_LEFT|IN_RIGHT))
			{
				...
				PrintColorText(client, "%s%sYour action was prevented for using %s+left/+right/+strafe%s. Your team was changed to %sspectate%s.",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					g_msg_textcol,
		...
		if(g_bTiming[client] == true)
		{
			...
			// Pausing
			if(GetEntityMoveType(client) == MOVETYPE_NONE && g_bPaused[client] != true){
				SetEntityMoveType(client, MOVETYPE_WALK);
			}else if(g_bPaused[client]){
		...
		if(Cvar_AllowAuto.BoolValue)
		{
			if(g_StyleConfig[Style][Auto] || (g_StyleConfig[Style][Freestyle] && g_StyleConfig[Style][Freestyle_Auto] && Timer_InsideZone(client, FREESTYLE, 1 << Style) != -1))
			{
				if((GetClientSettings(client) & AUTO_BHOP) && IsPlayerAlive(client))
				{
					if(buttons & IN_JUMP)
					{
						if((!(GetEntityFlags(client) & FL_ONGROUND)) && !(GetEntityFlags(client) & FL_PARTIALGROUND))
						{
		...
		if(Cvar_JumpInStartZone.BoolValue == false)
		{
			if(Timer_InsideZone(client, MAIN_START, -1) != -1 || Timer_InsideZone(client, BONUS_START, -1) != -1)
			{
				buttons &= ~IN_JUMP;
...
public Action:Event_UpdateNames(Handle:event, const String:name[], bool:dontBroadcast){
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
...
public Action:SM_BlockStrafe(client, args){
	if(IsClientInGame(client)){
		ForcePlayerSuicide(client);
		ChangeClientTeam(client, CS_TEAM_SPECTATOR);
		PrintColorText(client, "%s%sWe do not allow that command for %santi-cheat%s purposes!",
			g_msg_start,
			g_msg_textcol,
			g_msg_varcol,
			g_msg_textcol);
