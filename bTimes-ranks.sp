...
new bool:LoadPlayerStatsLate[MAXPLAYERS + 1];
...
public OnPluginStart(){
	...
	RegAdminCmd("sm_cclist", SM_CCList, ADMFLAG_CUSTOM5, "Shows a list of players with custom chat privileges.");
	RegAdminCmd("sm_recalcpts", SM_RecalcPts, ADMFLAG_ROOT, "Recalculates all the points in the database.");
	
	// Temp refresh-cc

	// Admin
	RegAdminCmd("sm_reloadranks", SM_ReloadRanks, ADMFLAG_ROOT, "Reloads chat ranks.");
...
{	
	...
	CreateTimer(5.0, UpdateDeaths, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	CreateTimer(15.0, EnableCCTimer, _, TIMER_FLAG_NO_MAPCHANGE | TIMER_REPEAT);
}
...
{
	if(g_bStatsLoaded != true){
		LoadPlayerStatsLate[client] = true;
	}else{
		LoadPlayerStatsLate[client] = false;
...
FormatTag(client, String:buffer[], maxlength){
	ReplaceMessage(buffer, maxlength);
	
	new rand[3], String:sRandHex[15];
	while(GetGameType() != GameType_CSGO && StrContains(buffer, "{rand}", true) != -1){
		for(new i=0; i<3; i++)
...
public Action:SM_Colorhelp(client, args)
{
	...
	PrintToConsole(client, "\nsm_ccname <arg> to set your name.\nsm_ccmsg <arg> to set your message.\n\nsm_ccname without an argument to turn colored name off\nsm_ccmsg without an argument to turn colored message off.\n");
	
	PrintToConsole(client, "Custom chat functions and operators:");
	...
	if(GetGameType() != GameType_CSGO){
		...
		PrintToConsole(client, "'^' followed by a hexadecimal code to use any custom color.\n--- Example: ^ffffff{name} -> produces a white name");
	}else
		PrintToConsole(client, "'^' followed by a number 1-10 or letter A-F (except D) will produce corresponding color.\n'^z' at the start of your message is required.\nONLY use this at the start of your cc-name and nowhere else.\n--- Example: ^z^10{name} -> produces an orange name");
	...
	if(idx != -1){
		GetArrayString(g_hCustomNames, idx, clientCustomName, MAXLENGTH_NAME);
		PrintToConsole(client, "\nYour current RAW cc-name:\n--- %s", clientCustomName);
	}
...
public Action:EnableCCTimer(Handle:timer){
	#if !defined SERVER
	for (new i = 0; i < MaxClients; i++){
		if(i != 0 && IsClientInGame(i)){
			decl String:cSteamID[32];
			GetClientAuthId(i, AuthId_Engine, cSteamID, sizeof(cSteamID));
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		if(SQL_GetRowCount(hndl) > 0)
		{
			...
			if(idx == -1)
			{
				if(client != 0)
					...
					PrintToServer("[Timer] A player with the name '%s' <%s> will be given custom chat privileges.",
						sName,
						sAuth);
			...
			else
			{
				if(client != 0){
					...
					PrintToServer("[Timer] A player with the given SteamID '%s' (name '%s') already has custom chat privileges.",
						sAuth,
						sName);
				}
			}
		...
		else
		{
			if(client != 0)
				...
				PrintToServer("[Timer] No player in the database found with '%s' as their SteamID.",
					sAuth);
...
//EnableCustomChat(const String:sAuth[], bool:console)
EnableCustomChat(const String:sAuth[]){
	...
	for(new client = 1; client <= MaxClients; client++)
	{
		if(client != 0 && IsClientInGame(client))
		{
			...
			if(StrEqual(sAuth, sAuth2)){
				...
				PrintColorText(client, "%s%sYou have been given custom chat privileges. Type %s!cchelp%s to learn how to use it.",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					g_msg_textcol);
				#if !defined SERVER
					PrintColorTextAll("%s%s%s%N%s has been automatically assigned %sdonator privileges%s! Thank you for donating!",
						g_msg_start,
						g_msg_textcol,
						g_msg_varcol,
						client,
	...
	if(GetGameType() == GameType_CSS)
		FormatEx(query, sizeof(query), "UPDATE players SET donator=1, ccuse=%d, ccname='{rand}{name}', ccmsgcol='^FFFFFF' WHERE SteamID='%s'",
			CC_HASCC|CC_MSGCOL|CC_NAME,
			sAuth);
	else
		FormatEx(query, sizeof(query), "UPDATE players SET donator=1, ccuse=%d, ccname='^z^7{name}', ccmsgcol='^1' WHERE SteamID='%s'",
			CC_HASCC|CC_MSGCOL|CC_NAME,
			sAuth);
	#endif
	...
	PushArrayString(g_hCustomNames, "^z^7{name}");
	PushArrayString(g_hCustomMessages, "^1");
...
DisableCustomChat(const String:sAuth[])
{
	...
	if(idx != -1)
	{
		...
		FormatEx(query, sizeof(query), "UPDATE players SET ccuse=0, WHERE SteamID='%s'",
			sAuth);
		#else
		FormatEx(query, sizeof(query), "UPDATE players SET ccuse=0, donator=0 WHERE SteamID='%s'",
			sAuth);
		#endif
...
public DB_ShowTopAll_Callback(Handle:owner, Handle:hndl, String:error[], any:data)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		for(new itemnum=1; itemnum<=rows; itemnum++)
		{
			...
			if(GetGameType() != GameType_CSGO && ((itemnum % 7 == 0) || (itemnum == rows)))
...
public LoadStats_Callback2(Handle:owner, Handle:hndl, String:error[], any:data)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		for(new Type; Type < MAX_TYPES; Type++)
		{
			for(new Style; Style < MAX_STYLES; Style++)
			{
				if(g_hMapsDoneHndlRef[Type][Style] != INVALID_HANDLE){
					// Close old handles
				...
				if(g_hMapsDone[Type][Style] != INVALID_HANDLE){
				...
				if(g_hRecordListID[Type][Style] != INVALID_HANDLE){
					ClearArray(g_hRecordListID[Type][Style]);}
				if(g_hRecordListCount[Type][Style] != INVALID_HANDLE){
...
public LoadRankList_Callback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		for(new client = 1; client <= MaxClients; client++)
		{
			...
			if(LoadPlayerStatsLate[client] == true)
			{
				SetClientRank(client);
				SetRecordCount(client);
				LoadPlayerStatsLate[client] = false;
